 ...
 * - disableClustering: Whether to disable the Cluster mode. default is false
 *
 ...
 * @return {TDAnalytics} SDK instance
 */
function initWithLoggingMode(path, config, log4jsConfig) {
    TDLog.info("consumer: log consumer. Log path: " + path);
    return new TDAnalytics(LoggingConsumer.init(path, config, log4jsConfig));
}

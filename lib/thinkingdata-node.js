...
let _createClient = function (consumer) {
    ...
    /**
     * 更新上报事件. 在上传事件数据（名称、属性）的时候，需要同时在参数对象中指明该事件关联的用户 ID.
     *
     * params 为参数对象，其属性包括:
     * - event: required {string} 事件名称
    ...
    /**
     * 覆盖上报事件. 在上传事件数据（名称、属性）的时候，需要同时在参数对象中指明该事件关联的用户 ID.
     *
     * params 为参数对象，其属性包括:
     * - event: required {string} 事件名称
     * - eventId : required {string} 事件ID
    ...
    /**
     * 首次事件. 在上传事件数据（名称、属性）的时候，需要同时在参数对象中指明该事件关联的首次检查 ID.
     *
     * params 为参数对象，其属性包括:
     * - event: required {string} 事件名称
     * - firstCheckId : required {string} 首次检查 ID
     * - accountId: optional 账号 ID
     * - distinctId: optional 访客 ID，账号 ID 和访客 ID 不能同时为空
     * - time: optional {Date} 事件时间
     * - ip: optional 事件发生的 IP 地址
...
module.exports = {
    ...
    /**
     * 初始化 Batch Mode 的实例。该实例会异步批量上报数据到服务端。
     * 
     * 默认情况下，数据会先缓存在内存中，当数据超过 20 条时会发起上报。定时3秒上报一次。
     * 退出的时候需要调用 ta.flush() 或者 ta.close() 保证数据上报.
     * 
     * @param {*} appId 
     * @param {*} serverUrl 
    ...
    /**
     * 获取 TA 内部 LoggerConsumer 使用的 log4js 配置文件
     *
     * 如果客户业务代码也使用到了 log4js 库，客户需要调用此方法获取 TA 的配置文件，与客户自有的配置文件融合。
     *
     */
    getLog4jsConfiguration: function () {
        return LoggingConsumer.getLog4jsConfiguration();
    },
};

...
_.properties = {
  _strip: function (prop) {
    ...
    _.each(prop, function (v, k) {
      if (!(_.check.isString(v) || _.check.isNumber(v) || _.check.isDate(v) || _.check.isBoolean(v) || _.check.isArray(v) )) {
        _.log('您的数据-', k, v, '-格式不满足要求，我们已经将其删除. 属性值只支持 String, Number, Date, Boolean, Array');
        delete prop[k];
      }
    });
    return prop;
